...
When testing components that use `tn-icon`, use the `TnIconTesting` utility to avoid manual mocking:
...
This provides working mocks for:
- `TnSpriteLoaderService` - Sprite loading and icon URLs
- `TnIconRegistryService` - Icon registration and resolution

**What you get:**
- Icons render as simple SVG elements (no fallback text)
- No errors from missing sprite files
- Clean text content for assertions
- All registry methods (registerLibrary, registerIcon, etc.) work

### Advanced: Customizing Mocks

If you need to customize icon behavior in tests, pass overrides to `providers()`:
...
**Why this approach?**
- Fresh mock instances on each call prevent test pollution
- No shared state between tests
- Partial overrides merge with sensible defaults

### Framework Support

Currently supports:
- **Jest** - `TnIconTesting.jest.providers()`

Future frameworks (Vitest, etc.) will follow the same pattern when added.
...
7. **Use TnIconTesting in tests** - Simplifies testing components with icons
