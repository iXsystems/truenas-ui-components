...
<ix-icon name="settings" library="material"></ix-icon>

<!-- Library custom icons (TrueNAS-specific) -->
<ix-icon name="dataset" library="custom"></ix-icon>
<ix-icon name="zvol" library="custom"></ix-icon>

<!-- Your app-specific custom icons -->
<ix-icon name="your-icon" library="custom"></ix-icon>
```
...
TrueNAS-specific icons included in the component library (ix- prefix)
...
**Usage:**
```typescript
iconMarker('your-icon', 'custom');  // Resolves to app-your-icon
```
...
If you use icons dynamically in TypeScript (not directly in templates), mark them with `iconMarker()`:
...
```html
<ix-icon name="my-logo" library="custom"></ix-icon>
```
...
1. **Scans TypeScript files** for `iconMarker()` and `libIconMarker()` calls
2. **Scans HTML templates** for `<ix-icon>` component usage
...
|---------|-----------|-------------|---------|
| MDI | `iconMarker('folder', 'mdi')` | `mdi-folder` | @mdi/svg package |
| Material | `iconMarker('home', 'material')` | `home` | @material-design-icons package |
| Library Custom | `iconMarker('dataset', 'custom')` | `ix-dataset` | Library assets |
| Consumer Custom | `iconMarker('my-logo', 'custom')` | `app-my-logo` | Your app assets |

This ensures library icons (`ix-*`) never conflict with your app icons (`app-*`).
...
Make sure you've marked them with `iconMarker()` and run `yarn icons` after adding the marker.

## Best Practices

1. **Use the sprite system for MDI and Material icons** - Don't register them manually
2. **Mark dynamic icons** - Use `iconMarker()` for icons used in TypeScript logic
