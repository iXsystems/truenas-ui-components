...
`--tn-fg1`, `--tn-fg2` and `--tn-alt-fg1` each clear the 4.5:1 text minimum (WCAG 1.4.3) against `--tn-bg1` and `--tn-bg2` in every theme, and `--tn-fg1` reads at least as well as `--tn-fg2` except in `.tn-midnight`, where `--tn-fg2` is the emphasis colour and both clear the minimum. That is the whole of the guarantee: it says nothing about `--tn-bg3` or either `--tn-alt-bg`, and text drawn on one of those — `tn-stepper` draws `--tn-alt-fg1` on `--tn-alt-bg1` — has to be measured against that surface rather than assumed from this one. `--tn-alt-fg2` is not covered either; it is drawn on the `--tn-alt-bg` surfaces rather than on the page. `text-fg-contrast.spec.ts` holds all nine palettes to what is claimed here.
