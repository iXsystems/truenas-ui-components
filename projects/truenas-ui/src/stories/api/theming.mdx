...
| the sortable table header's hover fill | **no** — it is a `color-mix` of the bar with its own label, not a token | `text-token-surface-contrast.spec.ts`, where four palettes are recorded below AA |
