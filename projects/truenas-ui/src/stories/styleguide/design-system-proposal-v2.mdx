...
### 🏗️ Information Architecture Revolution

- **Status-First Design**: Critical system health and alerts take visual priority
- **Hierarchical Clarity**: Clear visual relationships between pools → datasets → shares → files
...
- **Novice → Expert Progression**: Interfaces that grow with user knowledge
...
#### **tn-status-indicator: The Health Monitor**
...
💡 Tip: Most users start with a simple mirror (2 identical disks)
...
- **Progressive Tooltips**: Basic explanation → detailed explanation → expert details
...
This mockup shows how the task-oriented approach would look using actual tn-components from our library:

![High-Fidelity UI Mockup](/wireframes/high-fidelity-mockup.svg)

#### Key tn-Components Demonstrated

**tn-card as Information Hub**: The main storage setup card adapts its personality to welcome beginners while providing clear paths to advanced features.

**Progressive tn-button Components**:
...
**Smart tn-radio Selection**: Protection levels presented as user benefits rather than technical specifications, with progressive disclosure for VDEV details.

**Contextual tn-alert Components**: Smart recommendations that analyze user's drives and suggest optimal configurations.
...
3. **Component Reuse**: Same underlying `tn-card`, `tn-wizard`, and `tn-form-field` components
...
```
Current: "Create Pool" → "Pool Manager" → "Configure VDEVs"
Proposed: "Set Up Safe Storage" → "Choose Your Protection" → [Technical Toggle: "Pool/VDEV Configuration"]
```
...
Proposed:
- "Basic Protection" [Tech Toggle → "Mirror VDEV"]
- "Maximum Protection" [Tech Toggle → "RAIDZ2/RAIDZ3 VDEVs"]
- "Performance Boost" [Tech Toggle → "Cache VDEVs (L2ARC)"]
- "Write Speed" [Tech Toggle → "Log VDEVs (SLOG)"]
```
...
Proposed:
- "Safely Remove Storage" [Tech Toggle → "Export Pool"]
- "Add More Space" [Tech Toggle → "Expand Pool"]
- "Check Storage Health" [Tech Toggle → "Pool Scrub"]
```
...
Proposed:
- "Storage Overview" [Tech Toggle → "Pool Management"]
- "File Organization" [Tech Toggle → "Dataset Management"]
- "Backup History" [Tech Toggle → "Snapshot Management"]
```
...
- [ ] Redesign tn-card with required/optional element structure
- [ ] Implement tn-concept-explanation component
