import { Component, ViewChild, ElementRef, AfterViewInit, inject, Input, Output, EventEmitter, forwardRef } from '@angular/core';
...
export class IxCheckboxComponent implements AfterViewInit, ControlValueAccessor {
  ...
  ngAfterViewInit() {
    this.focusMonitor.monitor(this.checkboxEl)
      .subscribe(origin => {
        if (origin) {
          console.log(`Checkbox focused via: ${origin}`);
        }
      });
  }
